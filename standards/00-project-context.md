...
- **核心功能**：
  - **数据分析交互页面**：展示客户特征分布（年龄、职业、婚姻、教育等维度）、营销效果分析、数据探索与筛选
  - **在线预测系统**：基于离线训练的 ML 模型，通过点选式表单输入客户特征，实时预测该客户是否会认购定期存款，并给出概率与置信度
- **输入/数据**： 银行营销数据集（`data/train.csv`、`data/test.csv`），共 22 列 — 21 个特征（年龄、职业、婚姻、教育、联系方式、经济指标等），目标变量 `subscribe`（yes/no，是否认购定期存款）。**数据进 Git，非敏感教学数据**
...
| 构建 | `docker build` 成功 |
| 业务/模型指标 | 训练脚本输出模型指标（AUC ≥0.75、准确率），预测服务响应时间 <1s |
...
- CI 红灯不合并。
- 模型训练是离线操作（本地/Docker 构建时执行训练脚本），预测服务加载已训练模型。
